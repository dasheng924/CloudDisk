...
int recv_save_file(long len, char *user, char *filename, char *md5, long *p_size)
{
    ...
    //LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC,"%s,strlen = %d,sizeof=%u\n",tmp,strlen(tmp),sizeof(tmp));
    char *pEnd;
    *p_size = strtol(tmp, &pEnd, 10); //字符串转long
    //long rrret = atol(tmp);
    //LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC,"ret = %ld\n",rrret);
    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC,"*p_size = %ld,tmp = %s,pEnd = %s\n",*p_size,tmp,pEnd);
    begin = p;
    p = strstr(begin, "\r\n");
    p += 4;//\r\n\r\n
    len -= (p-begin);
