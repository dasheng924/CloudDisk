...
if [ "$START" -eq 1 ];then
    # 登录
    echo -n "登录："
    spawn-fcgi -a 10.211.55.11 -p 10000 -f ./bin_cgi/login
    # 注册
    echo -n "注册："
    spawn-fcgi -a 10.211.55.11 -p 10001 -f ./bin_cgi/register
    # 上传文件
    echo -n "上传："
    spawn-fcgi -a 10.211.55.11 -p 10002 -f ./bin_cgi/upload
    # MD5秒传
    echo -n "MD5："
    spawn-fcgi -a 10.211.55.11 -p 10003 -f ./bin_cgi/md5
    # 我的文件
    echo -n "MyFile："
    spawn-fcgi -a 10.211.55.11 -p 10004 -f ./bin_cgi/myfiles
    # 分享删除文件
    echo -n "DealFile："
    spawn-fcgi -a 10.211.55.11 -p 10005 -f ./bin_cgi/dealfile
    # 共享文件列表
    echo -n "ShareList："
    spawn-fcgi -a 10.211.55.11 -p 10006 -f ./bin_cgi/sharefiles
    # 共享文件pv字段处理、取消分享、转存文件
    echo -n "DealShare："
    spawn-fcgi -a 10.211.55.11 -p 10007 -f ./bin_cgi/dealsharefile
