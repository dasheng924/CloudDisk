...
void ShareList::addDownloadFiles()
{
    ...
    for(int i = 0; i < m_shareFileList.size(); ++i)
    {
        if(m_shareFileList.at(i)->item == item)
        {
            ...
            {
                logininfoinstance *login = logininfoinstance::getInstance(); // 获取单例
                m_cm.writeRecord(login->getUserName(), m_shareFileList.at(i)->filename, "010"); // 下载文件失败，记录
            }
            break; // 中断条件很重要
        }
    }
    ...
    connect(reply, &QNetworkReply::finished, [=]()
            {
                ...
                // 获取登陆信息实例
                logininfoinstance *login = logininfoinstance::getInstance(); // 获取单例
                m_cm.writeRecord(login->getUserName(), filename, "010"); // 下载文件成功，记录
...
void ShareList::dealFilePv(QString md5, QString filename)
{
    ...
    */
    QByteArray data = setShareFileJson( login->getUserName(),  md5, filename); // 设置json包
...
void ShareList::cancelShareFile(FileInfo *info)
{
    ...
    if(login->getUserName() != info->user)
    {
    ...
    */
    QByteArray data = setShareFileJson( login->getUserName(),  info->md5, info->filename); // 设置json包
